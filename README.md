...
- `slot_assigned?(:name)` → the plain presence check `(@name && @name != [])` (`:default` → `@inner_block`); both spread spellings (`{...expr}` and `...{expr}`) → `{expr}`; `:hook={"Name", from: Some.Mod}` with literal parts → the exact `phx-hook="Some.Mod#Name"`; LiveView modules used as tags (Surface's `live_render` sugar) → `{live_render(@socket, Mod, opts)}`; non-class comma/keyword sugar on any attr → a list literal (Surface's `:keyword`/`:list` semantics); profile `call_renames` maps Surface-era helper calls to their plain-stack analogues (Bonfire: `render_sface()` → `render_template()`)
...
**Elixir files**: `~F` → `~H` (bodies converted through the template pass), direct `use Surface.Component/LiveComponent/LiveView` → the `Phoenix` counterpart, and (mode-dependent) declarations to the most native construct available: `prop` → `attr` with type mapping for function components — including embedded-template modules, where a delegating `def render(assigns), do: ...` is emitted for the attrs to bind to — while stateful `prop`/`data` become [surf_live_attr](https://github.com/bonfire-networks/surf_live_attr)'s `live_attr` (Surface's reapply-on-update and internal-state semantics on plain LV; emitted commented-with-TODO if your project doesn't use the lib, so files always compile), `slot default` → `slot :inner_block` (`arg:` dropped; those semantics live in the caller's `:let`), `@doc` lines preceding a declaration folded into its `doc:` option (Surface's macros consumed `@doc`; Phoenix's don't — left alone they'd stack up and wrongly document `render/1`), `data` flagged. Declaration conversion only happens for declaration groups adjacent to a 1-arity def (dangling `attr` lines don't compile); everything else is flagged, not guessed.
